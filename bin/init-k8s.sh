...
kubectl -n zotprime exec -it $(kubectl -n zotprime get pods -l apps=zotprime-dataserver -o custom-columns=:metadata.name) -- sh -cux 'cd /var/www/zotero/admin && php schema_update'