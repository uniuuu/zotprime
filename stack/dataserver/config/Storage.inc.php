...
class Zotero_Storage {
	...
	public static function getUploadBaseURL() {
		$endpoint = !empty(Z_CONFIG::$S3_PUBLIC_ENDPOINT) ? Z_CONFIG::$S3_PUBLIC_ENDPOINT : 'http://' . Z_CONFIG::$S3_ENDPOINT;
		return $endpoint . "/" . Z_CONFIG::$S3_BUCKET . "/";
	...
	public static function updateFileItemInfo($item, $storageFileID, Zotero_StorageFileInfo $info, $client=false) {
		...
		$sql = "INSERT INTO storageFileItems (storageFileID, itemID, mtime, size) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE storageFileID = VALUES(storageFileID), mtime = VALUES(mtime), size = VALUES(size);";
		Zotero_DB::query(
			$sql,
			[
				$storageFileID,
